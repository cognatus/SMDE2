...
function showPopup(status, pop_up_id) {
	...
	if ( status ) {
		jQuery('.pop_up_container').css('z-index','99');
		selector.css({'left':'0','display':'none','z-index':'199'}).fadeIn();
		if ( $(window).width() >= 620 ) {
			selector.css('background-color','rgba(0,0,0,0.2)');
			selector.find('.pop_up').css({
