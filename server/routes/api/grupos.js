...
exports.createGroup = (req, res) => {
	...
	Course.findOneAndUpdate({ _id: courseId }, {
			...
			notif.insertNotifications({
				responsibleUsers: [req.decoded._id],
				action: {
					status: 2,
					substatus: 1,
...
exports.updateGroup = (req, res) => {
	...
	Course.findOne({ _id: courseId }).exec()
		.then( (doc) => {
			...
			notif.insertNotifications({
				responsibleUsers: [req.decoded._id], // Mandar a miembros viejos del grupo
				action: {
					status: 2,
					substatus: 2,
				...
				notif.insertNotifications({
					responsibleUsers: [req.decoded._id], //Mandar a miembros nuevos en el grupo
					action: {
						status: 2,
						substatus: 3,
						id: data._id,
